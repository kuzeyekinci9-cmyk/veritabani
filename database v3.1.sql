...
-- ============================================================================
-- GUI İÇİN GEREKLİ EK FONKSİYONLAR

CREATE OR REPLACE FUNCTION sp_get_my_tools(p_user_id INTEGER)
RETURNS TABLE(
    tool_id INTEGER,
    tool_name VARCHAR,
...
BEGIN
    RETURN QUERY
    SELECT t.tool_id, t.tool_name, c.category_name, t.status, t.tool_score
    FROM Tools t
    JOIN Categories c ON t.category_id = c.category_id
...
$$ LANGUAGE plpgsql;
 ---------------------
 CREATE OR REPLACE FUNCTION sp_update_tool_info(
    p_tool_id INTEGER,
